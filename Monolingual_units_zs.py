...
The final answer is: [number]"""
        ...
        output_csv = os.path.join(output_dir, f"qwen72_units_zs_prompt1_{level_num}{variant_name}.csv")
...
for level_num, df in levels.items():
    ...
    for variant_name, config in variants.items():
        ...
        output_csv = os.path.join(output_dir, f"qwen72_units_zs_prompt2_{level_num}{variant_name}.csv")
        pd.DataFrame(results).to_csv(output_csv, index=False)
        print(f"Results saved to {output_csv}")
...
for level_num, df in levels.items():
    ...
    for variant_name, config in variants.items():
        ...
        output_csv = os.path.join(output_dir, f"qwen72_units_zs_prompt3_{level_num}{variant_name}.csv")
        pd.DataFrame(results).to_csv(output_csv, index=False)
        print(f"Results saved to {output_csv}")
