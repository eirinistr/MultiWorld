...
def clean_text(text):
    """Καθαρίζει τα σύμβολα και τις αλλαγές γραμμής για καλύτερη μετάφραση."""
    text = text.replace("#", " ")
    text = text.replace("\n", " ")
...
LANG_CONFIG = {
    "en": {"answer": "Answer the following question:", "instruction": "DO NOT add any explanation.", "return_line": "Return ONLY one line.", "format": "OUTPUT FORMAT:", "final": "The final answer is: <number>", "examples": ["WorldA: The value of 1 minute is 100 seconds\nWorldB: The value of 1 minute is -50 seconds\nQuestion: How many seconds are in 2 minutes in WorldA?\nThe final answer is: 200", "WorldA: The value of 1 kilogram is 900 grams\nWorldB: The value of 1 kilogram is -100 grams\nQuestion: How many grams are in 3 kilograms in WorldB?\nThe final answer is: -300"]},

    "zh": {"answer": "请回答以下问题：", "instruction": "不要添加任何解释。", "return_line": "只返回一行。", "format": "输出格式：", "final": "最终答案是： <一个数字>", "examples": ["WorldA: 1 分钟的值是 100 秒\nWorldB: 1 分钟的值是 -50 秒\n问题: 在 WorldA 中，2 分钟是多少秒？\n最终答案是: 200", "WorldA: 1 千克的值是 900 克\nWorldB: 1 千克的值是 -100 克\n问题: 在 WorldB 中，3 千克是多少克？\n最终答案是: -300"]},

    "el": {"answer": "Απαντήστε στην ακόλουθη ερώτηση:", "instruction": "ΜΗΝ προσθέσετε καμία εξήγηση.", "return_line": "Επιστρέψτε ΜΟΝΟ μία γραμμή.", "format": "ΜΟΡΦΗ ΕΞΟΔΟΥ:", "final": "Η τελική απάντηση είναι: <ένας αριθμός>", "examples": ["WorldA: Η τιμή του 1 λεπτού είναι 100 δευτερόλεπτα\nWorldB: Η τιμή του 1 λεπτού είναι -50 δευτερόλεπτα\nΕρώτηση: Πόσα δευτερόλεπτα είναι 2 λεπτά στο WorldA;\nΗ τελική απάντηση είναι: 200", "WorldA: Η τιμή του 1 κιλού είναι 900 γραμμάρια\nWorldB: Η τιμή του 1 κιλού είναι -100 γραμμάρια\nΕρώτηση: Πόσα γραμμάρια είναι 3 κιλά στο WorldB;\nΗ τελική απάντηση είναι: -300"]},

    "tr": {"answer": "Aşağıdaki soruyu cevaplayın:", "instruction": "Herhangi bir açıklama eklemeyin.", "return_line": "SADECE tek satır döndürün.", "format": "ÇIKTI FORMATI:", "final": "Son cevap: <bir sayı>", "examples": ["WorldA: 1 dakikanın değeri 100 saniyedir\nWorldB: 1 dakikanın değeri -50 saniyedir\nSoru: WorldA'da 2 dakika kaç saniyedir?\nSon cevap: 200", "WorldA: 1 kilogramın değeri 900 gramdır\nWorldB: 1 kilogramın değeri -100 gramdır\nSoru: WorldB'de 3 kilogram kaç gramdır?\nSon cevap: -300"]},

    "sw": {"answer": "Jibu swali lifuatalo:", "instruction": "Usiongeze maelezo yoyote.", "return_line": "Rudisha mstari mmoja TU.", "format": "MUUNDO WA MATOKEO:", "final": "Jibu la mwisho ni: <nambari>", "examples": ["WorldA: Thamani ya dakika 1 ni sekunde 100\nWorldB: Thamani ya dakika 1 ni sekunde -50\nSwali: Dakika 2 ni sekunde ngapi katika WorldA?\nJibu la mwisho ni: 200", "WorldA: Thamani ya kilo 1 ni gramu 900\nWorldB: Thamani ya kilo 1 ni gramu -100\nSwali: Kilo 3 ni gramu ngapi katika WorldB?\nJibu la mwisho ni: -300"]},

    "ka": {"answer": "უპასუხეთ შემდეგ კითხვას:", "instruction": "არ დაამატოთ ახსნა.", "return_line": "დააბრუნეთ მხოლოდ ერთი ხაზი.", "format": "გამოტანის ფორმატი:", "final": "საბოლოო პასუხია: <რიცხვი>", "examples": ["WorldA: 1 წუთის მნიშვნელობა არის 100 წამი\nWorldB: 1 წუთის მნიშვნელობა არის -50 წამი\nკითხვა: WorldA-ში 2 წუთი რამდენი წამია?\nსაბოლოო პასუხია: 200", "WorldA: 1 კილოგრამის მნიშვნელობა არის 900 გრამი\nWorldB: 1 კილოგრამის მნიშვნელობა არის -100 გრამი\nკითხვა: WorldB-ში 3 კილოგრამი რამდენი გრამია?\nსაბოლოო პასუხია: -300"]}
}
def build_prompt(worldA_text, worldB_text, question, target_world, lang, other_lang):
    cfg = LANG_CONFIG[lang]
    other_cfg = LANG_CONFIG[other_lang]

    ex1 = cfg["examples"][0]
    ex2 = other_cfg["examples"][1]
...
df_level2 = pd.read_excel('untis.xlsx', sheet_name='level2', engine='openpyxl')
...
for level_num, df in levels.items():
    ...
    for variant_name, config in variants.items():
        ...
        output_csv = os.path.join(output_dir, f"qwen72_lang_units_fs_{level_num}{variant_name}.csv")
